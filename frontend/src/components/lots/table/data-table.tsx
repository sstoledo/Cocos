...
import { LotDataTableProps } from "@lots/types";
...
export function DataTableLot({ columns, data }: LotDataTableProps) {
  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);
  const [sorting, setSorting] = useState<SortingState>([
    { id: 'nameProduct', desc: false },
  ]);
            ...
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell
                  ...
                  className="h-24 text-center text-light-text-secondary dark:text-dark-text-secondary">
                  Sin resultados
                </TableCell>
              </TableRow>
            )}
