...
export const SelectClient: React.FC<ExtendedClientSelectProps> = (props) => {
  const hookResult = useClients();
  
  const clients = props.clientsList || hookResult.clients;
  ...
  const showNoClient = mode === 'update';

  return (
    <Select
      onValueChange={(value) => onSelect(value === "no-clients" ? null : value)}
      value={selectedValue || (showNoClient ? "no-clients" : undefined)}
    >
      <SelectTrigger
        className="w-full
