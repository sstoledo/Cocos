...
export class AuthService {
  ...
  async login(loginUserDto:LoginUserDto) {
    ...
    try {
      ...
      const comparePassword = await this.encryptionService.compare(password,user.password);

      if(!comparePassword) throw new UnauthorizedException('Credentials are not valid (password)');
